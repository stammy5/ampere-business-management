...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalRevenue = await prisma.clientInvoice.aggregate({
      where: {
        status: { in: ["PAID"] }
      },
      _sum: {
    ...
    const currentMonthRevenue = await prisma.clientInvoice.aggregate({
      where: {
        status: { in: ["PAID"] },
        paidDate: {
    ...
    const lastMonthRevenue = await prisma.clientInvoice.aggregate({
      where: {
        status: { in: ["PAID"] },
        paidDate: {
          gte: lastMonthStart,
    ...
    const pendingInvoices = await prisma.clientInvoice.count({
      where: {
    ...
    const pendingValue = await prisma.clientInvoice.aggregate({
      where: {
        status: { in: ["SENT", "DRAFT"] }
      },
      _sum: {
    ...
    const currentQuarterClients = await prisma.client.count({
      where: {
        createdAt: {
          gte: quarterStart,
          lte: now
    ...
    const lastQuarterClients = await prisma.client.count({
      where: {
        createdAt: {
